...
SkyBoxSphere::SkyBoxSphere(float x, float y, float z, LPCWSTR texturePath, Camera* camera, ID3D11Device* device_ptr, ID3D11DeviceContext* device_context_ptr)
{
	LPCWSTR pixelPath;
	...
	char* spherePath = new char[126]{ "SkyBoxSphere.obj" };
...
void SkyBoxSphere::setPosition(float x, float y, float z)
{
...
void SkyBoxSphere::scaleSize(float x, float y, float z)
{
	shader->setModelMatrix(XMMatrixScaling(x, y, z) * XMMatrixTranslationFromVector(this->posVec));

}

void SkyBoxSphere::draw(ID3D11Buffer* light_constant_buffer, ID3D11Buffer* lightNr_constant_buffer,
